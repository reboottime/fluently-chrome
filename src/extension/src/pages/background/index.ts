import { Message, MESSAGE_ACTIONS, VoiceMessage } from "@utils/messageUtils";
  ...
  private forwardVoiceMesage = () => {
    chrome.runtime.onMessage.addListener(async (message, _sender, sendResponse) => {
      if (message.action === MESSAGE_ACTIONS.SEND_VOICE_MESSAGE) {
        try {
          const forwardedMessage = new Message(MESSAGE_ACTIONS.RENDER_VOICE_MESSAGE, message.data)
